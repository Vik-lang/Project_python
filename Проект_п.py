class Array:
    """Создание двустронней очереди с помощью простого массива"""
    def __init__(self, card):
        """Создаем объект - копию списка"""
        self.data = card

    def has_cards(self):
        """метод has_cards определяет есть ли у игрока карты в колоде """
        return bool(self.data) # возвращает False, если колода пуста, иначе True

    def draw_card(self):
        """метод draw_card позволяет игроку вытянуть карту из колоды"""
        return self.data.pop(0)  # удаляем и возвращаем элемент с начала
...
class Game:
    ...
    def __init__(self, p1_cards, p2_cards):
        self.player1 = Array(p1_cards)
        self.player2 = Array(p2_cards)
...
print('Значение вывода: second (номер раунда) - выиграл 2 игрок (в каком-то раунде), ')
print('first - выиграл 1 игрок (в каком-то раунде), botva - количество раундов было превышено.\n\n')
